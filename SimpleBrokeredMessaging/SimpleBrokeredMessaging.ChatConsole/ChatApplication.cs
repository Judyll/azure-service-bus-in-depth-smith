...
namespace SimpleBrokeredMessaging.ChatConsole
{
    class ChatApplication
    {
        ...
        static readonly string _topicPath = "chat-test-topic";
        static void Main(string[] args)
        {
            ...
            var manager = new ManagementClient(_serviceBusConnectionString);

            // Create a topic if it does not exist            
            if (!manager.TopicExistsAsync(_topicPath).Result)
            {
                manager.CreateTopicAsync(_topicPath).Wait();
            }

            // Create a subscription for the user
            var description = new SubscriptionDescription(_topicPath, userName)
            {
            ...
            // Create clients
            var topicClient = new TopicClient(_serviceBusConnectionString, _topicPath);
            var subscriptionClient = new SubscriptionClient(_serviceBusConnectionString, _topicPath, userName);
